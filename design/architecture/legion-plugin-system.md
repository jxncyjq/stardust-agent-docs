...
| ~~**G2 Guest SDK**~~ | §6.10 承诺的 `pkg/legionplugin`（Go/wasip1）+ `sdk/rust/legion-plugin` crate；`plugin_example` 已迁到 SDK（五个文件降为一个） | ✅ **已交付** 分支 `feat/plugin-guest-sdk`；计划 `plans/2026-08-28-plugin-guest-sdk.md` | 写插件不再需要手搓 ABI 与 JSON |
